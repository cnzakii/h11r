...
- Python's `Connection.buffered_bytes` reports how many received bytes are held
  but not yet parsed. `trailing_data` already exposed those bytes, but copied
  them on every access. A count lets an adapter that pushes bytes in as they
  arrive apply read back-pressure without materializing the backlog.
