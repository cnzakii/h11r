...
def main() -> None:
    ...
    runner = pyperf.Runner(
        metadata={
            "http11_version": candidate.__version__,
    ...
    runner.argparser.add_argument("--implementation", choices=("http11", "h11"))
    args = runner.parse_args()
    for name, parts in SCENARIOS:
        if args.scenario and args.scenario != name:
            continue
        if args.implementation in (None, "http11"):
            runner.bench_func(f"scenario/{name}/http11", CandidateServerCycle(parts))
    ...
    if args.scenario in (None, "client_server_round_trip"):
        if args.implementation in (None, "http11"):
            runner.bench_func(
                "scenario/client_server_round_trip/http11", CandidateRoundTrip()
            )
        if args.implementation in (None, "h11"):
            runner.bench_func(
