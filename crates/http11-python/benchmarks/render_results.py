...
def render_svg(rows: list[tuple[str, float, float]], details: str) -> str:
    ...
    speedups = [h11 / http11 for _, http11, h11 in rows]
    ...
    svg = [
        ...
        '<title id="title">http11 and h11 Python benchmark</title>',
    ...
    for index, (label, http11, h11) in enumerate(rows):
        speedup = h11 / http11
        y = 124 + index * row_height
        h11_width = plot_width / axis_max
        http11_width = plot_width * speedup / axis_max
        svg.extend(
            [
                ...
                f"{http11 * 1e6:.2f} µs vs {h11 * 1e6:.2f} µs</text>",
                f'<text class="secondary" x="278" y="{y + 12}" text-anchor="end" '
                'font-size="10">http11</text>',
                f'<rect class="candidate" x="{plot_x}" y="{y}" width="{http11_width:.1f}" '
                'height="15" rx="7.5"/>',
                f'<text class="primary" x="{min(plot_x + http11_width + 10, 920):.1f}" '
...
def render_table(rows: list[tuple[str, float, float]], details: str) -> str:
    lines = [
        "| Scenario | http11 (µs/op) | h11 0.16.0 (µs/op) | Relative throughput |",
        "| --- | ---: | ---: | ---: |",
    ]
    for label, http11, h11 in rows:
        lines.append(
            f"| {label} | {http11 * 1e6:.2f} | {h11 * 1e6:.2f} | {h11 / http11:.1f}× |"
        )
    if details:
