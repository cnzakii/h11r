...
def test_request_target_rejects_bytes_outside_its_field_boundary(
    ...
    connection = http11.Connection(http11.Role.CLIENT)
    with pytest.raises(http11.LocalProtocolError):
...
def test_response_octets_and_folded_fields_cross_the_python_boundary() -> None:
    ...
    assert isinstance(response, http11.Response)
...
def test_request_framing_errors_keep_the_rfc_status_across_python() -> None:
    ...
    with pytest.raises(http11.RemoteProtocolError) as raised:
...
def test_python_text_and_buffer_inputs_preserve_http_octets() -> None:
    ...
    invalid_text = http11.Connection(http11.Role.CLIENT)
    with pytest.raises(ValueError, match="ASCII"):
        invalid_text.send_request("GET", "/", (("Host", "é.example"),))

    invalid_pair = http11.Connection(http11.Role.CLIENT)
...
def test_direct_api_and_receive_events() -> None:
    ...
    assert connection.next_event() == http11.EndOfMessage()
    assert connection.next_event() is http11.ReceiveStatus.NEED_DATA

    assert connection.send_response(204, reason=b"No Content") == (
        b"HTTP/1.1 204 No Content\r\n\r\n"
    )
    assert connection.end_of_message() == b""
    assert connection.local_state is http11.State.DONE
    assert connection.peer_state is http11.State.DONE
...
def test_continue_body_trailers_and_reuse_cross_the_python_boundary() -> None:
    ...
    client = http11.Connection(http11.Role.CLIENT)
    server = http11.Connection(http11.Role.SERVER)
    ...
    assert isinstance(server.next_event(), http11.Request)
    ...
    assert isinstance(informational, http11.InformationalResponse)
    ...
    assert isinstance(body, http11.Data) and body.data == b"body"
    ...
    assert isinstance(client.next_event(), http11.Response)
    assert isinstance(client.next_event(), http11.EndOfMessage)
    client.start_next_cycle()
    server.start_next_cycle()
    assert client.local_state is client.peer_state is http11.State.IDLE
    assert server.local_state is server.peer_state is http11.State.IDLE
...
def test_upgrade_pause_and_trailing_data_cross_the_python_boundary() -> None:
    ...
    assert isinstance(server.next_event(), http11.Request)
    assert isinstance(server.next_event(), http11.EndOfMessage)
    assert server.next_event() is http11.ReceiveStatus.PAUSED
    ...
    assert client.local_state is client.peer_state is http11.State.SWITCHED_PROTOCOL
    assert server.local_state is server.peer_state is http11.State.SWITCHED_PROTOCOL
...
def test_connection_close_event_crosses_the_python_boundary() -> None:
    ...
    assert isinstance(connection.next_event(), http11.ConnectionClosed)
    assert connection.trailing_data == (b"", True)
    assert connection.local_state is http11.State.MUST_CLOSE
    assert connection.peer_state is http11.State.CLOSED
    connection.close()
    assert connection.local_state is connection.peer_state is http11.State.CLOSED
...
def test_limits_and_remote_error_status() -> None:
    connection = http11.Connection(http11.Role.SERVER, max_header_count=1)
    connection.receive_data(b"GET / HTTP/1.1\r\nHost: x\r\nX: y\r\n\r\n")
    try:
        connection.next_event()
    except http11.RemoteProtocolError as error:
...
def test_events_have_value_equality_and_parts_require_contiguous_data() -> None:
    wire = b"GET / HTTP/1.1\r\nHost: x\r\n\r\n"
    first = http11.Connection(http11.Role.SERVER)
    second = http11.Connection(http11.Role.SERVER)
...
def test_event_properties_reuse_their_immutable_python_values() -> None:
    ...
    assert isinstance(body, http11.Data)
